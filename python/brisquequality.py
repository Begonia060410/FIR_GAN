...
def AGGDfit(structdis):
    ...
    # calculate left sigma variance and right sigma variance  #nan开始了，0/0
    if negcount == 0 or poscount == 0:
        print('-')
    else:
    ...
    # vectorized function call for best fitting parameters # changenp.float64
...
def test_img(img):
    ...
    for itr_scale in range(scalenum):
        ...
        sigma = np.abs((sigma - mu_sq)) ** 0.5

        # structdis is the MSCN image
        structdis = im - mu
    if not (np.any(structdis)):
        return False
    else:
        return True
